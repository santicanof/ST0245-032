...
public class Quadtree
{
    ...
    public int numColisiones() {
        return numColisiones(this.root.n, 0) + colEspeciales();
    }

    private int numColisiones(Nodo [] n, int nodos) {
        ...
        if (n[nodos].a.size() < 2) {
            return numColisiones(n, nodos + 1);
        } else if(n[nodos].a.size() == 2) {
            double dist = n[nodos].a.get(0).calcular(n[nodos].a.get(1));
            if(dist <= min) {
                return 1 + numColisiones(n, nodos + 1);
            } else {
                return numColisiones(n, nodos + 1);
            }          
        } else if(n[nodos].diagonal() <= min) {
            return (int)combinatoria(n[nodos].a.size()) + numColisiones(n, nodos + 1); 
        } else {
            return numColisiones(n[nodos].dividir(), 0) + numColisiones(n, nodos + 1);
        }
    }

    public int colEspeciales() {
        return colisiones(this.root.hor) + colisiones(this.root.ver);
    }

    public int colisiones(ArrayList <Abeja> a) {
        int colisiones = 0;
        double min = 9.000009 * pow(10, -4);
